...
2. plugin-development-agentがagent-generatorスキルを使用して、ユーザーとの対話を通じてエージェントの責任範囲、役割、注意事項などの情報を収集する（interaction-guidelinesに従う）
3. plugin-development-agentがagent-generatorスキルを使用して、収集した情報を基にエージェントファイルのコンテンツを生成する（element-definition-conventionに従う）
4. plugin-development-agentがagent-generatorスキルを使用して、markdownlint検証を実施する（documentation-standardsに従う）
...
- `[プラグインディレクトリ]/agents/[エージェント名].md`: エージェント定義ファイル
