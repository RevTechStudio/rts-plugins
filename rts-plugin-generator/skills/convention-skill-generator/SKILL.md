...
- SKILL.mdファイルの生成
- 設計原則の遵守確認（他要素への参照禁止、固有名詞禁止、重複最小化）
- markdownlint検証の実施
- ユーザーフィードバックの収集と反映

## 設計原則

生成するコンベンションスキルは、以下の設計原則を遵守する必要がある:

### 独立性の原則

- 他のスキル、エージェント、コマンドを参照してはいけない
- 必要な情報はすべてスキル内に直接記述する
- スキル単体で完結し、他の要素に依存しない

### 汎用性の原則

- 固有名詞（組織名、プロジェクト名、製品名）を使用してはいけない
- 特定の環境やコンテキストに依存しない
- 汎用的なプレースホルダーを使用する（例: YourProject, YourCompany）
...
- 既存のスキルと重複する内容を含めない
- スキル生成前に既存スキルを確認し、同じ目的のスキルが存在しないか確認する
- 重複する記述がある場合は、既存スキルをコマンド側で参照する設計にする
- コンテキストサイズを最小化するため、他スキルで提供される内容は記述しない
- 必要な場合のみ新規スキルを作成し、既存スキルで十分な場合は作成しない
...
```markdown
【スキル概要の確認】
このコンベンションスキルが定義する規約・ガイドラインの概要を教えてください。
例: 「コーディング規約」「ドキュメント記述標準」「セキュリティガイドライン」

【対象範囲の確認】
この規約・ガイドラインは以下のどの範囲をカバーしますか？
1. 特定領域のみ（例: 命名規則のみ、フォーマットのみ）
2. 複数領域（例: 命名規則 + フォーマット + コメント規則）【推奨】
3. 包括的（すべての側面をカバー）

【カテゴリ分類の確認】
規約・ガイドラインを以下のようなカテゴリに分類できますか？
例: 「命名規則」「コードレイアウト」「例外処理」など
```
...
- 良い例/悪い例の提示が理解を助ける
- 詳細なチェックリストが必要
- 複数のバリエーションやパターンを示す必要がある

**質問例:**

```markdown
【テンプレート選択】
収集した情報から、以下のテンプレートを推奨します：

1. simple版（基本構造のみ、シンプルで読みやすい）【推奨】
2. full版（詳細なカテゴリ説明、良い例/悪い例を含む）

この規約の複雑度から、simple版を推奨しますが、どちらで作成しますか？
```
...
- ❌ 他のスキル名、エージェント名、コマンド名を参照しない
- ❌ 固有名詞（組織名、プロジェクト名、製品名）を使用しない
- ✅ 汎用的な表現を使用する（例: 「プロジェクト」「組織」）
- ✅ 必要な規約やガイドラインはスキル内に直接記述する

**重複最小化の確認:**

既存スキルと重複する内容が含まれていないかを確認する:

- 対話パターンや質問技法は含めない
- Markdown記述ルールやmarkdownlint検証の詳細は含めない
- 既存のConvention Skillと重複する規約は含めない
- スキル固有の内容のみを記述する
...
```markdown
このSkillは、XYZ社のコーディング規約を定義する
```

**対処法:**

- 固有名詞を削除する
- 汎用的な表現に置き換える（例: 「このSkillは、コーディング規約を定義する」）
