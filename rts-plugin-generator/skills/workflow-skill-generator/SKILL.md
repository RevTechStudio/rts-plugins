...
生成するワークフロースキルは、以下の設計原則を遵守する必要がある:
...
```markdown
【スキル概要の確認】
このワークフロースキルが実行する作業の概要を教えてください。
例: 「設計書からソースコードを生成する」「APIドキュメントを作成する」

【作業フローの確認】
この作業は以下のどのパターンに近いですか？
1. 単一フェーズ（一連の作業を一度に実行）
2. 複数フェーズ（段階的に作業を実行）【推奨】
3. 条件分岐あり（状況に応じて処理が変わる）

【入力・出力の確認】
- 入力データ: このスキルが必要とする情報や前提条件は何ですか？
- 出力データ: このスキルが生成する成果物は何ですか？
```
...
2. full版（詳細なフェーズ説明、良い例/悪い例、エラー処理を含む）

このスキルの複雑度から、simple版を推奨しますが、どちらで作成しますか？
```
...
```markdown
このSkillは、XYZ社のプロジェクトで使用される
```

**対処法:**

- 固有名詞を削除する
- 汎用的な表現に置き換える（例: 「このSkillは、すべてのプロジェクトで使用される」）
